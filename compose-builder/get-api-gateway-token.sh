...
docker run --rm -it -e KONGURL_SERVER=edgex-kong -e "ID=${ID}" -e "JWT_FILE=${JWT_FILE}" --network compose-builder_edgex-network --entrypoint "" -v ${GW_KEY_DIR}:/keys -v ${JWT_VOLUME}:${JWT_VOLUME} \
...
docker run --rm -it -e KONGURL_SERVER=edgex-kong -e "ID=${ID}" --network compose-builder_edgex-network --entrypoint "" -v ${GW_KEY_DIR}:/keys \
       ${CORE_EDGEX_REPOSITORY}/security-proxy-setup${ARCH}:${CORE_EDGEX_VERSION}${DEV} \
